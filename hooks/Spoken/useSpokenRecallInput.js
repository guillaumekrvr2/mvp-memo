...
/**
 * Formate le texte spoken avec espaces Unicode et retours à la ligne forcés tous les N caractères
 ...
 */
function formatSpokenDisplay(text, cols = 6) {
...
/**
 * Nettoie le texte d'input utilisateur pour spoken numbers
...
/**
 * Hook pour gérer l'input de rappel de spoken numbers avec support des tirets pour skip
 ...
 */
export default function useSpokenRecallInput(objectif, cols = 6, lineHeight = 36) {
  ...
  const handleInputChange = useCallback((text) => {
    const cleanText = cleanSpokenInput(text, objectif)
    setUserInput(cleanText)
  }, [objectif])

  // Formate le texte pour l'affichage
  const displayValue = useMemo(() => {
    return formatSpokenDisplay(userInput, cols)
  }, [userInput, cols])
