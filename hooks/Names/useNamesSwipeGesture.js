...
import { withSpring, withTiming, runOnJS, withDelay } from 'react-native-reanimated'
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      if (isTopProfile) {
        ...
        rotateZ.value = -10 + (event.translationX / 10)
      }
    })
    .onEnd((event) => {
      ...
      if (isTopProfile && isSwipeOut) {
        ...
        // Animation de sortie avec CALLBACK IMMÉDIAT comme Cards
        ...
        // ✨ CALLBACK IMMÉDIAT comme Cards (pas d'attente)
        runOnJS(onSwipe)()
      } else {
